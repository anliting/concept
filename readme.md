...
<body>
<script type=module>
import{Root,component,dom,useState}from'https://cdn.jsdelivr.net/gh/anliting/concept/main/main.mjs'
let{button}=dom,root=new Root
...
Call `useState` in a component to declare a state variable:

```
let[state,setState]=useState(initialState)
```

`initialState` is the value of the state initially. If `useState` is called with no arguments, i.e. `useState()`, it is treated as `useState(undefined)`.

Call `setState` with a function to set it to another value:
...
The function accepts the previous value, and returns the next value.

If you want to set it to a value that does not depend on the previous value, like `setState(a=>1)`, you can pass the value directly, like `setState(1)`.
